...
def save_to_csv(data : list):
    ...
    csv_file = './blackbox_data.csv'
    headers = [ 'gyro_xout', 'gyro_yout', 'gyro_zout', 
...
class MPUSensor:
    ...
    def __init__(self, smbus_line : int, i2c_addr : int, refresh_freq : int, blackbox_data_size : int, record_black_box = False) -> None:
        ...
        # Blackbox data init
        self.record_black_box = record_black_box
        self.blackbox_data_size = blackbox_data_size
        self.blackbox_data = []
    ...
    def run(self):
        try:
            while True:
                ...
                if self.record_black_box:
                    data_set = (gyro_xout, gyro_yout, gyro_zout, 
                                gyro_xout_scaled, gyro_yout_scaled, gyro_zout_scaled,
                                accel_xout, accel_yout, accel_zout, 
                                accel_xout_scaled, accel_yout_scaled, accel_zout_scaled,
                                rot_x, rot_y
                                )
                    self.blackbox_data.append(data_set)
                    if len(self.blackbox_data) > self.blackbox_data_size:
                        self.blackbox_data.pop(0)
            ...
            if self.record_black_box:
                save_to_csv(self.blackbox_data)
